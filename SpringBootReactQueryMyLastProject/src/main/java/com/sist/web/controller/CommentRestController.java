...
public class CommentRestController {
	...
	@DeleteMapping("/comment/delete/{no}/{sno}")
	public ResponseEntity<Map> comment_delete(@PathVariable("no") int no,@PathVariable("sno") int sno)
	{
		Map map=new HashMap();
		try {
			
			List<CommentVO> list=cService.commentDelete(no, sno);
			map.put("comments", list); 
		} catch (Exception e) {
			// TODO: handle exception
			e.printStackTrace();
	...
	public ResponseEntity<Map> comment_update(@RequestBody CommentEntity vo) {
	    ...
	    try {
	        ...
	        List<CommentVO> list = cService.commentUpdate(no, msg, sno);
	        map.put("comments", list);
	        return new ResponseEntity<>(map, HttpStatus.OK);
	    } catch (Exception e) {
	        e.printStackTrace();
